...
const upload = multer({ storage, limits: { fileSize: 5 * 1024 * 1024 }, fileFilter: (req, file, cb) => {
  // allow common image types and audio files (mp3, wav, ogg, etc.)
...
app.post('/api/chat/upload', upload.single('image'), async (req, res) => {
  try {
    ...
    try {
      // Only attempt image processing for image mime types; skip for audio
...
app.post('/api/threads', writeLimiter, upload.single('image'), async (req, res) => {
  ...
  if (req.file && req.file.filename) {
    const fname = req.file.filename
    const filePath = path.join(uploadsDir, fname)
    // if the upload was audio, just store the url; do not run image processing
